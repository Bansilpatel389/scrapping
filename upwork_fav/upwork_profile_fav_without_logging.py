...
from seleniumbase import SB          # UC Mode
from selenium.webdriver.common.by import By
...
# -------------------------------------------------
# Main scraping logic with SeleniumBase UC Mode
# -------------------------------------------------

def execute(sb, id, id_type):
    global ids_count
    print('id : ', id)
    url = 'https://www.upwork.com/jobs/~' + str(id)

    time_module.sleep(random.uniform(2.5, 6.0))

    # ---- Open with UC reconnect (best Cloudflare method) ----
    try:
        sb.uc_open_with_reconnect(url, reconnect_time=6)
    except Exception as e:
        print(f"[UC] uc_open_with_reconnect failed: {e}")
        return

    # ---- Try to clear Cloudflare if still present ----
    try:
        title = (sb.get_title() or "").lower()
        if "just a moment" in title or "checking your browser" in title:
            print("[CF] Challenge detected → trying uc_gui_click_captcha")
            sb.uc_gui_click_captcha()          # OS-level click (works under Xvfb)
            time_module.sleep(random.uniform(3, 5))

            # Second try if needed
            title = (sb.get_title() or "").lower()
            if "just a moment" in title:
                print("[CF] Still on challenge → uc_gui_handle_captcha")
                sb.uc_gui_handle_captcha()
                time_module.sleep(random.uniform(3, 5))
    except Exception as e:
        print(f"[CF] Captcha handling error: {e}")

    # Final check
    title = (sb.get_title() or "").lower()
    if "just a moment" in title or "checking your browser" in title:
        print("[CF] Still blocked after UC methods – skipping this ID")
        return

    # ---- Extract NUXT data ----
    try:
        nuxt = NUXT_function(sb.driver)          # pass the underlying driver
    except Exception as e:
        print(f"NUXT extraction failed: {e}")
        return

    # ---- Closed / Private / Removed checks (same as before) ----
    try:
        try:
            status = nuxt['vuex'][1]['jobDetails']['job']['status']
            if status == 2:
        ...
        try:
            status_text = sb.get_text('//*[@id="main"]/div/div/div/div[1]/div[2]/h4')
        except Exception:
            try:
                status_text = sb.get_text('//*[@id="main"]/div[3]/div[4]/h1')
            except Exception:
                status_text = ""

        status_lower = status_text.lower()
        if 'no longer available' in status_lower or 'not found' in status_lower:
            set_is_removed(id, 1)
            return
    ...
    time_module.sleep(random.uniform(1.0, 2.5))
...
def priority(sb):
    ...
    if hourly_ids:
        print(f"Updating hourly {len(hourly_ids)} fav id")
        for id in hourly_ids:
            execute(sb, id, "hourly")
            ids_count += 1

def upwork_specific(sb, daily_ids):
    global ids_count
    ids_count = 0
    try:
        priority(sb)
...
try:
    # SeleniumBase context manager – creates a stealthy UC browser
    with SB(
        uc=True,                    # Undetected Chrome Mode
        test=False,
        headless=False,             # Must be False for uc_gui_click_captcha under Xvfb
        xvfb=True,                  # SeleniumBase will manage Xvfb if needed
        ...
        while True:
            try:
                ...
                daily_ids = []          # keep disabled for now
                # daily_ids = fetch_daily_ids()
